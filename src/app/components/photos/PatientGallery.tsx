...
// Nota: He ajustado la URL para que siempre apunte a la base de la API.
...
export default function PatientGalleryWithWelcome() {
  ...
  const initSession = async () => {
    try {
      // ✅ MODIFICACIÓN: Obtener datos de autenticación desde localStorage
      ...
      if (!sesionResponse.ok) {
        // Mejor manejo de error si el token es inválido o expiró
        if (sesionResponse.status === 401 || sesionResponse.status === 403) {
      ...
      if (sesionData.idPaciente !== idUsuario) {
      ...
      // Si el error es de autenticación, forzamos la redirección
  ...
  const handleSaveDescription = async () => {
    ...
    try {
      // ✅ MODIFICACIÓN: Obtener token desde localStorage
      const token = localStorage.getItem("authToken")
      if (!token) {
        throw new Error('Sesión no válida o expirada. Redirigiendo a login.')
      }
      ...
      if (esUltimaFoto && sesionActiva) {
        ...
        try {
          const updateResponse = await fetch(
            `${API_URL}/descripciones-imagenes/actualizarSesion/${sesionActiva.idSesion}`,
            {
              method: 'PATCH',
              headers: {
                'Content-Type': 'application/json',
